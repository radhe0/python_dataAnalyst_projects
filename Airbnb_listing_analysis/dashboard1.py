...
if filtered_data.empty:
    ...
    st.subheader("💲 Most Expensive Listings")
    expensive_listings = filtered_data.nlargest(10, 'price')[['name', 'host_name', 'neighbourhood_group', 'room_type', 'price']]
    st.write(expensive_listings)
    st.markdown("---")

    # Listings with Highest Availability
    st.subheader("🟢 Listings with the Highest Availability")
    high_availability = filtered_data.nlargest(10, 'availability_365')[['name', 'host_name', 'neighbourhood_group', 'room_type', 'availability_365']]
    st.write(high_availability)
    st.markdown("---")
